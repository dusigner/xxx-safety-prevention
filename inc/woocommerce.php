...
function xxx_safety_wc_render_classic_cart_template( $content ) {
	if ( is_admin() || is_feed() || ! function_exists( 'is_cart' ) || ! is_cart() || ! function_exists( 'WC' ) ) {
		return $content;
	}
	...
	$uses_cart_block = has_block( 'woocommerce/cart', $post ) || false !== strpos( $content, 'wp-block-woocommerce-cart' );
	if ( ! $uses_cart_block ) {
		return $content;
	}
...
function xxx_safety_wc_render_classic_checkout_template( $content ) {
	...
	$uses_checkout_block = has_block( 'woocommerce/checkout', $post ) || false !== strpos( $content, 'wp-block-woocommerce-checkout' );
...
add_filter( 'the_content', 'xxx_safety_wc_render_classic_checkout_template', 8 );
